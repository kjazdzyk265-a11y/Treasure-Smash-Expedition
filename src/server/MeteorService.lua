...
local function label(root: BasePart)
    ...
    title.Text = "GLOBAL GIANT METEOR"
    title.TextColor3 = Color3.fromRGB(255, 125, 54)
    ...
    hp.Name = "HPText"
    hp.Size = UDim2.fromScale(1, 0.45)
    hp.Position = UDim2.fromScale(0, 0.55)
...
local function updateLabel()
    ...
    local hpText = activeMeteor.PrimaryPart:FindFirstChildOfClass("BillboardGui")
    local labelObj = hpText and hpText:FindFirstChild("HPText")
    if labelObj and labelObj:IsA("TextLabel") then
        labelObj.Text = string.format("%d / %d HP", math.max(0, math.floor(activeMeteor:GetAttribute("Health") or 0)), maxHealth)
    end
end
...
local function awardParticipants()
    for player, damage in pairs(contributions) do
        if player.Parent == Players and damage > 0 then
            ...
            if gemValue and gemValue:IsA("IntValue") then
                gemValue.Value += gems
                feedbackRemote:FireClient(player, "REWARD", gems, share)
            end
        end
    end
end
...
function MeteorService.Spawn()
    ...
    contributions = {}
    totalDamage = 0
    maxHealth = METEOR_HEALTH * math.max(1, #Players:GetPlayers())

    local model = Instance.new("Model")
    model.Name = "GiantMeteor"
    model:SetAttribute("EventMeteor", true)
    model:SetAttribute("Health", maxHealth)
    model:SetAttribute("MaxHealth", maxHealth)
    model:SetAttribute("Alive", true)
    model:SetAttribute("Zone", 1)

    local root = Instance.new("Part")
    root.Name = "MeteorCore"
    root.Shape = Enum.PartType.Ball
    root.Size = Vector3.new(24, 24, 24)
    root.Position = Vector3.new(0, 13, 18)
    root.Anchored = true
    root.Material = Enum.Material.Slate
    root.Color = Color3.fromRGB(69, 45, 39)
    root.TopSurface = Enum.SurfaceType.Studs
    root.BottomSurface = Enum.SurfaceType.Inlet
    root.Parent = model

    local glow = Instance.new("PointLight")
    glow.Color = Color3.fromRGB(255, 91, 37)
    glow.Range = 35
    glow.Brightness = 4
    glow.Parent = root
    ...
    activeMeteor = model
    updateLabel()
    stateRemote:FireAllClients("STARTED", maxHealth, maxHealth, 0)
end
...
function MeteorService.ApplyDamage(player: Player, model: Model, requestedDamage: number)
    ...
    contributions[player] = (contributions[player] or 0) + actual
    totalDamage += actual
    local newHealth = health - actual
    model:SetAttribute("Health", newHealth)
    updateLabel()
    stateRemote:FireAllClients("DAMAGE", newHealth, maxHealth, totalDamage)
    if newHealth <= 0 then
