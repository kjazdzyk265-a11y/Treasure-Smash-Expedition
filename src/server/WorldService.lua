...
local function spawnBreakable(parent, kind, position, scale)
    ...
    local model=Instance.new("Model"); model.Name=kind; model:SetAttribute("BreakableType",kind); model:SetAttribute("Health",cfg.MaxHealth); model:SetAttribute("MaxHealth",cfg.MaxHealth); model:SetAttribute("Reward",cfg.Reward); model:SetAttribute("Alive",true); model:SetAttribute("Boss",cfg.Boss==true); model:SetAttribute("Zone",cfg.Zone or 1)
    ...
    if cfg.Boss then label(root, string.upper(kind), Color3.fromRGB(255,230,85)) end
...
local function buildLandmark(zoneFolder, zone)
    ...
    for i=1,4 do
        local x=(i<=2 and -1 or 1)*9; local z=(i%2==0 and -1 or 1)*9
...
local function buildDecor(zoneFolder, zone)
    ...
    for i=1,14 do
        ...
        local x=side*(55+(i%3)*7); local z=-66+i*9
        if zone.Id==1 then
...
local function buildZone(world, zone)
    ...
    buildDecor(folder,zone); buildLandmark(folder,zone)

    for i=1,28 do
        local side=i%2==0 and 1 or -1; local z=-62+(i%14)*9; local x=side*(20+((i*7)%4)*8)
        local kind=zone.Breakables[(i-1)%#zone.Breakables+1]
        spawnBreakable(folder,kind,zone.Offset+Vector3.new(x,2.5,z))
    end
    ...
    local prompt=Instance.new("ProximityPrompt"); prompt.ActionText="Open Egg"; prompt.ObjectText=zone.Name.." Egg"; prompt.MaxActivationDistance=12; prompt.HoldDuration=0; prompt.Parent=egg
    ...
    if zone.Id<#Zones then
        local gate=part(folder,"Gate",Vector3.new(6,18,40),zone.Offset+Vector3.new(95,9,0),zone.Accent,Enum.Material.Neon); gate:SetAttribute("UnlockZone",zone.Id+1); gate:SetAttribute("UnlockCost",Zones[zone.Id+1].UnlockCost); label(gate,"ZONE "..(zone.Id+1),Color3.new(1,1,1))
    end
end
